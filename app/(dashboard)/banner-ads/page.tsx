...
const createAd = async (formData: FormData, token: string | undefined): Promise<Ad> => {
  if (!token) throw new Error("No token found");

  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/banner-ads`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
const updateAd = async ({ id, formData, token }: { id: string; formData: FormData; token: string | undefined }): Promise<Ad> => {
  if (!token) throw new Error("No token found");
...
const deleteAd = async (id: string, token: string | undefined): Promise<void> => {
  if (!token) throw new Error("No token found");
  ...
  if (!response.ok) {
    const errorData = await response.json();
    const errorMessage = errorData.errorSources?.[0]?.message || errorData.message || "Failed to delete ad";
...
export default function BannerAdsPage() {
  ...
  const { data: ads, isLoading, error, isError } = useQuery<Ad[], Error>({
    queryKey: ["ads"],
  ...
  const createMutation = useMutation({
    mutationFn: (formData: FormData) => createAd(formData, session?.accessToken),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Banner Ads Management</h1>
          <nav className="text-sm text-[#272727]">Dashboard {">"} Banner Ads</nav>
        </div>
        <button
          onClick={openModal}
        ...
        >
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm">
            ...
            <p className="text-sm text-gray-700 mb-6">
              Are you sure you want to delete banner ad with ID {adToDelete}? This action cannot be undone.
            </p>
            <div className="flex space-x-4">
              <button
      ...
      <div className="p-6 rounded-lg">
        ...
        {Array.isArray(ads) && ads.length === 0 && (
          <div className="text-center py-4 text-[#272727]">No ads available</div>
        )}
        {Array.isArray(ads) && ads.length > 0 && (
          <div className="overflow-x-auto">
